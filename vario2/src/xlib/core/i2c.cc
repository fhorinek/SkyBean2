...
void I2c::InitMaster()
{
	this->InitMaster(i2cC, 100000ul, BUFFER_SIZE, BUFFER_SIZE);
}
...
void I2c::InitSlave()
{
	this->InitSlave(i2cC, i2c_defaultSlave, BUFFER_SIZE, BUFFER_SIZE);
}

void I2c::InitSlave(uint8_t slaveAddress)
{
	this->InitSlave(i2cC, slaveAddress, BUFFER_SIZE, BUFFER_SIZE);
}
...
ISR(TWIC_TWIM_vect) { i2cs[0]->IrqRequest(); }
ISR(TWIC_TWIS_vect) { i2cs[0]->IrqSlaveRequest(); }
